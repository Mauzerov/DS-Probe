...
# All required missing no plots and stuff
...
outliers = dirty_iris['Sepal.Length'][
    np.abs(
        dirty_iris['Sepal.Length'] - dirty_iris['Sepal.Length'].mean()
    ) > (
        1.5 * dirty_iris['Sepal.Length'].std()
    )
]
...
encoded_categorical = pd.DataFrame(
    encoder.fit_transform(dirty_iris[categorical_cols]),
...
carseats_numeric = carseats.select_dtypes(include=[np.number])
carseats[carseats_numeric.columns] = imputer.fit_transform(carseats_numeric)

sns.boxplot(carseats)
```
...
```python
# your code goes here
income_bins = pd.qcut(carseats["Income"],10)
income_bins
```
    ...
    Categories (10, interval[float64, right]): [(20.999, 30.0] < (30.0, 39.0] < (39.0, 48.0] < (48.0, 62.0] ... (77.0, 84.6] < (84.6, 96.2] < (96.2, 107.0] < (107.0, 120.0]]
...
```
    ...
    Number of complete cases: 400
    Percentage of complete cases: 100.00%



```python
# Find outliers in carseats.column
outliers_idx = pd.Index([]);
outliers_columns = []
for column in carseats.select_dtypes(include=[np.number]).columns[:1]:
    outliers = carseats[column][
        np.abs(carseats[column] - carseats[column].mean()) > (2 * carseats[column].std())
    ]
    idxs = carseats.index[carseats[column].isin(outliers)]
    if idxs.size:
        outliers_columns.append(column)
        outliers_idx = outliers_idx.append(idxs)
# Print the rows with outliers
print("Outliers:")
carseats.loc[outliers_idx]
```
...
for column in outliers_columns:
    plt.boxplot(carseats[column].dropna())
    plt.title('Boxplot of Outliers Columns')
    plt.ylabel(column)
    plt.grid()
plt.show()
```
